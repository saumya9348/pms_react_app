     ...
     if(req.method==='GET' && curUrl.pathname==="/products"){ //get specific array obj
        if(checkNull(reqId)){
            ...
            if(checkNull(reqId)){
                ...
                res.end(JSON.stringify({status:true,message:"Delete sucessfully"}));
            }else{
                res.end(JSON.stringify({status:false,message:"ID required"}));
            }
    }
        ...
        if(checkNull(reqId)){
            ...
            res.write(JSON.stringify({status:200,message:"Sucessfully Updated"}));
            res.end();
        }else{
            res.end(JSON.stringify({status:404,message:"ID required"}));
        }
    }
        ...
        req.on("end",()=>{
            if(checkNull(data)){
                ...
                res.write(JSON.stringify({status:200,message:"Sucessfully added"}));
                res.end();
            }else{
                res.end(JSON.stringify({status:204,message:"Please enter all the fields"}));
            }
        })
